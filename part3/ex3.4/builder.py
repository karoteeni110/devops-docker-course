...
def clone_repository(github_repo):
    subprocess.run(["git", "clone", "https://github.com/" + github_repo + ".git"])
...
def publish_to_docker_hub(repository):
    ...
    subprocess.run(["docker", "login", "-u", DOCKER_USER, "-p", DOCKER_PWD])
    subprocess.run(["docker", "push", repository])

if __name__ == "__main__":
    # Check if the required arguments are provided
    if len(sys.argv) != 3:
        print("Usage: ./builder.py <GitHub Username>/<Repo Name> <Docker Hub Username>/<Repo Name>")
        sys.exit(1)
